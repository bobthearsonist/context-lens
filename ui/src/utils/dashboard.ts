...
export function computeDashboardKpis(summaries: ConversationSummary[], scope: KpiScope = 'all'): DashboardKpis {
  // When scoped to today, use the `*Since` deltas (today's portion) instead of
  // cumulative lifetime totals — sessions resumed across days would otherwise
  // re-count prior-day activity in the "today" KPIs.
  const scoped = scope === 'today'
  const totalSessions = summaries.length
  const totalRequests = summaries.reduce((sum, summary) => sum + (scoped ? summary.entriesSince : summary.entryCount), 0)
  const totalTokens = summaries.reduce((sum, summary) => sum + summary.latestTotalTokens, 0)
  const totalCost = summaries.reduce((sum, summary) => sum + (scoped ? summary.costSince : summary.totalCost), 0)
